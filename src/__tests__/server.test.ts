...
describe('createServer', () => {
  ...
  test('registers exactly five tools: whoami, eser_tescil, sozlesme_durumu, sablonlarim, sablon_detay', () => {
    const server = createServer(makeTestOpts('imz_x'));
    const tools = (server as unknown as PrivateServer)._registeredTools;
    const names = Object.keys(tools).sort();
    expect(names).toEqual(['eser_tescil', 'sablon_detay', 'sablonlarim', 'sozlesme_durumu', 'whoami']);
  });
