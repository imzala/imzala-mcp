...
export const sablondanSozlesmeInputSchema = {
  ...
  idempotency_key: z.string().optional().describe('Aynı isteği tekrarlamamak için benzersiz anahtar (retry koruması)'),
};
...
export function registerSablondanSozlesme(server: McpServer, resolveClient: ResolveClient): void {
  server.registerTool(
    ...
    {
      description:
        'Bir şablondan yeni sözleşme oluşturur. 1 kredi harcar (geri alınamaz). Varsayılan olarak sözleşmeyi oluşturur ama davet GÖNDERMEZ; imza linklerini taraflara siz iletirsiniz. `gonder: true` verirseniz taraflara hemen gerçek SMS ve e-posta daveti gönderilir. Hangi template_party_id ve değişkenlerin gerektiğini sablon_detay aracıyla öğrenin. (Yapay zeka asistanına: başarısızlıkta bu aracı körü körüne TEKRAR çağırma, ikinci bir sözleşme ve ikinci kredi oluşturur.)',
